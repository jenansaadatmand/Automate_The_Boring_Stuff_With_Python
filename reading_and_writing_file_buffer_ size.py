...
while len(msg):   # Loop through file 10 bytes at a time, checks len variable msg, as long as the value is not 0 the loop will run
    ...
    msg = inputFile.read(10) #Writes a message to output file, after writing the message, a statement will read the next 10 bytes and keeps doing it until entire file is read
inputFile.close()
outputFile.close()
